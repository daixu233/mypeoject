  ...
  <div class="bank-bind">
      ...
      <div class="info-block distance">
          <div class="info-line input">
              ...
              <div class="info-ipt">
                  <cube-input type="number" v-model="bankId" @input="bankChange" :maxlength="18" placeholder="请输入银行卡号" ></cube-input>
              </div>
          </div>
          <div class="info-line input">
              <span class="info-label">所属银行</span>
              <div class="info-ipt">
                  <cube-input :value="back_name" disabled ></cube-input>
                  <!-- <cube-select
                    v-model="type"
      ...
      <div v-if="hasBankInfo" class="bank-tips">
      ...
      <div class="info-block distance">
          <div class="info-line input">
              ...
              <div class="info-ipt">
                  <cube-input type="number" v-model="phone" placeholder="请输入持卡人手机号码" ></cube-input>
              </div>
          </div>
          <div class="info-line input">
              <span class="info-label">验证码</span>
              <div class="info-ipt short">
                    <cube-input v-model="validate_code" placeholder="请输入短信验证码" ></cube-input>
              </div>
      ...
      <div class="bot-btn" @click="confirmBind">确认绑定</div>
  </div>
</template>
  ...
  methods: {
      ...
      bankChange(event) {
          if (/^([1-9]{1})(\d{15}|\d{18})$/.test(this.bankId)) {
              this.axios.post('/api/loan/check-card', {
      ...
      getValidCode() {
        ...
        this.validSwitch()
        this.axios.post('/api/loan/send-bind-card-code', {
            phone: this.phone,
            card_num: this.bankId,
...
.bank-bind .cube-input:after, .bank-bind .cube-select:after {
    border: none!important;
}
.bank-bind .cube-input, .bank-bind .cube-select {
    font-size: .32rem;
}
